...
void HandleSnakeInput(Snake* snake) {
    ...
    if (keyHit) {
        ...
        // 2. Prevent redundant directions (Ignoring the same direction reduces input lag)
        if ((nextDir.x != 0 && nextDir.x != currentDir.x && nextDir.x != -currentDir.x) ||
            (nextDir.y != 0 && nextDir.y != currentDir.y && nextDir.y != -currentDir.y)) {
