...
void SpawnFood(Food *food, Snake *snake, GameData *game) {
  // Stage 1: Random Sampling (Fast - O(1) expected)
  // Most of the time, the grid is empty enough that random guessing finds a spot instantly.
  for (int attempts = 0; attempts < 64; attempts++) {
    int rx = GetRandomValue(0, GRID_COUNT_X - 1);
    int ry = GetRandomValue(0, GRID_COUNT_Y - 1);

    // Check map (Avoid walls and portals)
    if (game->map[rx][ry] != TILE_EMPTY) continue;
  ...
  // Stage 2: Exhaustive Fallback (Guaranteed - O(Cells))
  // If random sampling fails 64 times (grid is mostly full), we find all empty tiles.
  ...
  for (int x = 0; x < GRID_COUNT_X; x++) {
    for (int y = 0; y < GRID_COUNT_Y; y++) {
      // Check map (Avoid walls and portals)
      if (game->map[x][y] != TILE_EMPTY) continue;
  ...
  if (emptyCount > 0) {
    ...
    // Grid is completely full (Win condition or no space). 
    // Move food off-screen to prevent further iterations.
...
void DrawFood(Food *food, Texture2D fruitTex) {
  Vector2 center = {food->position.x * GRID_SIZE + (GRID_SIZE / 2.0f),
