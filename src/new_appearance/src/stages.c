...
void LoadStage(GameData *game, int level) {
    ...
    if (level == 2) {
        ...
        for (int x = 0; x < GRID_COUNT_X; x++) {
            for (int y = 0; y < GRID_COUNT_Y; y++) {
                bool isBorder = (x < 2 || x >= GRID_COUNT_X - 2 || y < 2 || y >= GRID_COUNT_Y - 2);
                int mid = GRID_COUNT_X / 2;
                bool isGateX = (x >= mid - 2 && x <= mid + 1);
                bool isGateY = (y >= mid - 2 && y <= mid + 1);
                if (isBorder) {
                    if ((x < 2 || x >= GRID_COUNT_X - 2) && isGateY) continue;
                    if ((y < 2 || y >= GRID_COUNT_Y - 2) && isGateX) continue;
                    game->map[x][y] = TILE_WALL;
                }
            }
        }
        ...
        for (int i = 0; i < len; i++) {
            ...
            game->map[GRID_COUNT_X - 1 - offset - i][GRID_COUNT_Y - 1 - offset] = TILE_WALL;
            game->map[GRID_COUNT_X - 1 - offset][GRID_COUNT_Y - 1 - offset - i] = TILE_WALL;
        }
    }
}
