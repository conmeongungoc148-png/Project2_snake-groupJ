...
void InitSnake(Snake *snake) {
  ...
  snake->timePerMove = 0.125f; // Initial default matching 8 FPS
...
void UpdateSnakeLogic(Snake *snake, GameData *game) {
  ...
  // Terminal Wall Portal (Grid Wrapping)
...
bool CheckCollisionWithSelfOrWall(Snake *snake, GameData *game) {
  if (snake->invincibilityTimer > 0) return false;
  Vector2 head = snake->body[0];

  // Grid bounds check (Disabled - Portals active)
  /*
...
void DrawSnake(Snake *snake, Texture2D headTex, Texture2D bodyTex,
  ...
  for (int i = snake->length - 1; i >= 0; i--) {
    ...
    if (snake->invincibilityTimer > 0) {
        float flash = sinf(GetTime() * 20.0f);
        if (flash > 0) alpha /= 2; // Semi-transparent flash
    }
